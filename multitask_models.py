...
def create_multitask_modules(shared_module_defs, diff_module_defs_list):
    ...
    for i, module_def in enumerate(shared_module_defs):
        ...
        modules, filters = parse_module(i, module_def)
    ...
    for j, diff_module_defs in enumerate(diff_module_defs_list):
        for i, module_def in enumerate(diff_module_defs):
            modules, _ = parse_module(i, module_def)
...
class MultiDarknet(nn.Module):
    ...
    def __init__(self, shared_config_path, diff_config_paths, img_size=416):
        ...
        self.shared_module_defs = parse_model_config(shared_config_path)
        self.diff_module_defs_list = [parse_model_config(i) for i in diff_config_paths]
        self.hyperparams, self.module_list_shared, self.module_list_diff = create_multitask_modules(self.shared_module_defs, self.diff_module_defs_list)
        self.img_size = img_size
        self.seen = 0
        self.header_info = np.array([0, 0, 0, self.seen, 0])
        self.loss_names = ["x", "y", "w", "h", "conf", "cls", "recall", "precision"]
    ...
    def forward(self, x, cond, targets=None):
        ...
        self.losses = defaultdict(float)
        layer_outputs = []
        module_defs = self.shared_module_defs + self.diff_module_defs_list[cond]
        module_list = self.module_list_shared + self.module_list_diff[cond]
